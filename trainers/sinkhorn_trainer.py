...
class SinkhornTrainer(BaseTrainer):
    ...
    def validate_on_train(self):
        ...
        clf = KMeansLabelGenerator(4, False, 64)
        preds = clf.fit(feats)
    ...
    def validate_on_test(self):
        ...
        for x, _ in tqdm(self.test_generator):
            pred = self.model.compute_features(x)
    ...
    def train(self, epochs=20, class_weight=None, callbacks=[], watcher=None):
        ...
        for ep in range(epochs):
            ...
            print(f'ep {ep}$ Train acc: {train_acc}, Train f1: {train_f1}')
            ...
            watcher.insert_batch_results([ f1_train_scores, accuracy_train_scores])
            ...
            watcher.insert_batch_results([f1_max_train, accuracy_max_train])
